...
**⚠️ Después de cualquier cambio de código (como la reforma de agosto 2026), la implementación web no se actualiza sola.** `probarApiPublica` corre el código directo desde el editor y siempre refleja la versión actual — pero la URL `/exec` real puede seguir sirviendo una versión de código *anterior* hasta que alguien vaya a Implementar > Administrar implementaciones > (ícono de lápiz) > Versión: Nueva versión > Implementar. Antes de confiar en que la landing ve los datos post-reforma, verificar la URL real en incógnito, no solo el resultado de `probarApiPublica`.

### 3.2 Forma exacta de la respuesta

Capturada de una ejecución real del sistema, ya después de la reforma de agosto 2026 (los ítems ING-0031/ING-0032/ING-0033/ING-0035 son datos de prueba a limpiar antes de ir a producción — ver §3.5):

```json
{
  "recaudado": 6958287,
  "recibido_en_especie": 5001,
  "utilizado": 1050622,
  "saldo": 5907665,
  "donaciones": 8,
  "gastos": 5,
  ...
  "operaciones": [
    ...
    },
    {
      ...
      "fecha": "2026-08-13",
      "tipo": "EGRESO",
      "categoria": "INSUMOS/EQUIPAMIENTO",
      "concepto": "Compra de insumos de primeros auxilios",
      "importe": 88000,
      "comprobante": "https://drive.google.com/comprobante-egr-03",
      "tipo_recurso": "-"
    }
  ],
  "entregas_detalle": [],
  "impacto_por_categoria": [
    {
      "categoria": "ALIMENTOS",
      "mujeres": 0, "infancias": 0, "diversidades": 0, "varones": 0,
      "animales": 0, "total_personas": 0
    },
    {
      "categoria": "ALOJAMIENTO TEMPORAL",
      "mujeres": 1, "infancias": 1, "diversidades": 0, "varones": 0,
      "animales": 1, "total_personas": 2
    }
  ]
}
```
...
- **`tipo_recurso` es nuevo:** para ingresos vale `"MONETARIO"` o `"ESPECIE"`; para egresos siempre `"-"` (no aplica). Una donación en especie (ej. una estufa a gas, insumos) queda registrada y visible, pero **no suma** a `recaudado` ni a `saldo` — aparece aparte en `recibido_en_especie`. La landing debería mostrar un badge distinto (ej. "EN ESPECIE") en los ítems de `operaciones` donde `tipo_recurso === "ESPECIE"`.
- **`beneficiarios_mujeres`, `beneficiarios_infancias`, `beneficiarios_diversidades`, `beneficiarios_varones`, `animales_beneficiados` son nuevos:** desglose por impacto, sumando egresos y entregas verificados. `beneficiarios` sigue siendo el total de personas (sin contar animales). Todo esto refleja el pedido explícito de la organización de medir impacto por género, infancias y bienestar animal — es información con peso propio, no un detalle menor.
- **`impacto_por_categoria` es nuevo:** array de **siempre 12 objetos**, uno por cada categoría de ayuda posible (ALIMENTOS, AGUA, MEDICAMENTOS, INSUMOS/EQUIPAMIENTO, TRANSPORTE, LOGISTICA, ALOJAMIENTO TEMPORAL, COMUNICACION, AYUDA ECONOMICA DIRECTA, ROPA, PAP APOYO EMOCIONAL, OTROS — en ese orden fijo), con el mismo desglose demográfico pero cruzado por categoría. Combina Egresos (columna Categoría) y Entregas (columna Tipo_de_ayuda) como si fueran el mismo eje — una categoría puede sumar 0 en una hoja y tener datos en la otra, es normal. Las categorías sin ningún registro verificado vienen con todos los campos en 0, no ausentes del array — no hay que filtrar por "existe o no", siempre están las 12.
- `comprobante` puede venir como **string vacío** (`""`) — pasa seguido en cargas por Forms (campo opcional). No asumir que siempre hay un link para mostrar.
- `entregas_detalle` puede venir como **array vacío** — la campaña recién arranca. La UI tiene que verse bien con 0 entregas, no solo con datos de ejemplo.
...
- **Categorías de egreso actualizadas:** ALIMENTOS, AGUA, MEDICAMENTOS, INSUMOS/EQUIPAMIENTO, TRANSPORTE, LOGISTICA, ALOJAMIENTO TEMPORAL, COMUNICACION, AYUDA ECONOMICA DIRECTA, OTROS (10 en total — si el Gráfico 1 de §3.4 tenía la lista vieja hardcodeada en algún lado, hay que actualizarla).
...
- Recibido en especie (`recibido_en_especie`) — mostrarlo cerca de "Recaudado" pero visualmente separado (ej. una tarjeta secundaria, no sumada a la principal), con una aclaración corta tipo "bienes e insumos donados, no se contabilizan como dinero disponible".
- Donaciones (`donaciones`), Gastos (`gastos`), Entregas (`entregas`) — grilla secundaria de contadores.
...
Agrupar `operaciones` donde `tipo === "EGRESO"` por `categoria`, sumando `importe`. Categorías posibles (§3.2): ALIMENTOS, AGUA, MEDICAMENTOS, INSUMOS/EQUIPAMIENTO, TRANSPORTE, LOGISTICA, ALOJAMIENTO TEMPORAL, COMUNICACION, AYUDA ECONOMICA DIRECTA, OTROS. Es el gráfico que más responde a "¿en qué se gastó la plata?".

**Gráfico 2 — Recaudación acumulada en el tiempo (línea o área):**
Ordenar `operaciones` donde `tipo === "INGRESO"` por `fecha`, sumar `importe` en acumulado (solo `tipo_recurso === "MONETARIO"`, para no mezclar con especie en una curva que se supone es de dinero). Muestra la curva de crecimiento de la campaña.

**Gráfico 3 — Recaudado vs. Utilizado (barra comparativa simple o gauge):**
Dos barras (`recaudado` vs `utilizado`) o una barra de progreso mostrando `saldo` como remanente.

**Gráfico 4 — Impacto por población alcanzada, total general (NUEVO — es el pedido central de la Red):**
Barras o íconos-contador con `beneficiarios_mujeres`, `beneficiarios_infancias`, `beneficiarios_diversidades`, `beneficiarios_varones` y, aparte (no mezclado con personas), `animales_beneficiados`. Esto responde directamente a por qué existen estos campos: la organización necesita poder mostrar, no solo cuánta plata movió, sino a quién ayudó y con qué enfoque de género/diversidad/bienestar animal. Conviene que esta sección tenga tanto peso visual como los números financieros — no un detalle chico al pie.

**Gráfico 5 — Impacto cruzado por categoría (NUEVO, complementa al Gráfico 4):**
Usa `impacto_por_categoria` directamente — no requiere ningún cálculo client-side, ya viene agregado. La forma más legible para 12 categorías × 5 dimensiones no es 12 gráficos de torta sueltos: mejor una **tabla o mapa de calor** con las categorías como filas y Mujeres/Infancias/Diversidades/Varones/Animales como columnas, con intensidad de color proporcional al valor de cada celda (o simplemente el número, con las celdas en 0 bien apagadas/grises para no competir visualmente con las que sí tienen datos). Alternativa más gráfica si se prefiere: barras apiladas horizontales, una por categoría, con un segmento de color por dimensión demográfica — pero con 12 categorías posibles, filtrar o colapsar primero las que están en 0 (mostrar "aún sin datos de impacto en esta categoría" en vez de una fila de barra vacía) para que el gráfico no quede dominado por ceros.

**Feed de operaciones verificadas:**
Lista/tabla de `operaciones` (más recientes primero por `fecha`), con badge de color por `tipo` (verde para INGRESO, el acento que se defina para EGRESO), **más un segundo badge cuando `tipo_recurso === "ESPECIE"`** (ej. "EN ESPECIE", con un ícono de caja/paquete en vez de billete), mostrando `concepto`, `importe` formateado en pesos colombianos, y link a `comprobante` solo si no viene vacío.
...
- Si todos los indicadores de impacto (`beneficiarios_*`, `animales_beneficiados`) están en 0 — pasa hoy mismo, recién se sumó el desglose — el Gráfico 4 no debería verse como una sección rota. Un estado tipo "Todavía no hay datos de impacto cargados" es mejor que 4 barras en cero sin contexto.
- Con pocos registros verificados, la mayoría de las 12 categorías en `impacto_por_categoria` van a estar en 0 (es lo esperado hoy) — el Gráfico 5 no debería mostrar 12 filas grises apagadas como protagonistas; mejor destacar solo las categorías con datos reales y dejar el resto colapsado o en una lista secundaria ("+ 9 categorías sin datos todavía").
- Si `operaciones` tuviera muy pocos registros, el Gráfico 2 (línea temporal) puede verse pobre con 1-2 puntos — contemplar un mensaje tipo "la campaña recién arranca".

### 3.5 Antes de ir a producción

Hay 4 filas de prueba cargadas en el sistema real (`ING-0031` "donacion" $10.000, `ING-0032` "harina" $5.000 en especie, `ING-0033` "aporte solidario" $4.000.000, `ING-0035` "estufa a gas" $1 en especie) marcadas como verificadas y públicas. **Hay que volver a la hoja 01_INGRESOS y pasarlas a Publicar=NO (o borrarlas) antes de anunciar la landing**, para no arrancar la campaña con datos ficticios en el contador de recaudación.
...
- [ ] **Actualizar la implementación del Web App** (Implementar > Administrar implementaciones > editar > Nueva versión) para que la URL `/exec` real sirva el código posterior a la reforma — probarApiPublica no es prueba suficiente de esto, hay que chequear la URL en incógnito (§3.1)
- [ ] Filas de prueba (ING-0031/32/33/35, incluidos $4.000.000 ficticios y una "estufa a gas" en especie) despublicadas o borradas en `01_INGRESOS` — **lo más urgente**, ya que el endpoint está en vivo
